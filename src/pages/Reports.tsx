  ...
  <div className="flex flex-col min-w-[180px] print:min-w-[200px] w-full">
    ...
    <div className="relative">
      ...
      <input
        ...
        className="w-full h-11 pr-3 pl-9 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 print:h-auto print:py-1.5 print:rounded-md print:border-gray-300"
      />
    </div>
  </div>
);
...
const Reports: React.FC = () => {
  ...
  return (
    <div className="p-6 pb-safe bg-gray-50 min-h-screen">
      <div className="flex flex-col gap-3 mb-6 md:flex-row md:items-end md:justify-between print:flex-row print:items-center print:justify-between print:mb-3">
        <h1 className="text-2xl font-bold text-gray-900 print:text-[20pt] print:font-extrabold">Relatórios</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <StatCard title="Atendimentos Hoje" value={`${todayCompletedAppointments}/${todayTotalAppointments + todayScheduledSlots.length}`} icon={Clock} color="blue" />
        <StatCard title="Atendimentos Semana" value={`${weekCompletedAppointments}/${weekTotalAppointments}`} icon={Calendar} color="blue" />
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6 print:shadow-none print:border-gray-300 print:rounded-md print:break-inside-avoid">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Resumo do Dia</h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center"><p className="text-blue-600 font-semibold text-2xl">{todayCompletedAppointments}</p><p className="text-gray-500 text-sm">Concluídos</p></div>
          <div className="text-center"><p className="text-yellow-600 font-semibold text-2xl">{todayScheduledSlots.length}</p><p className="text-gray-500 text-sm">Agendados</p></div>
          <div className="text-center"><p className="text-orange-600 font-semibold text-2xl">{todayHistory.filter(h => h.status === 'no_show').length}</p><p className="text-gray-500 text-sm">Faltaram</p></div>
          <div className="text-center"><p className="text-green-600 font-semibold text-2xl">R$ {todayHistory.filter(h => h.status === 'concluido').reduce((sum, h) => sum + (h.price * (h.clinicPercentage / 100)), 0).toFixed(0)}</p><p className="text-gray-500 text-sm">Receita</p></div>
          <div className="text-center"><p className="text-purple-600 font-semibold text-2xl">{todayTotalAppointments > 0 ? ((todayCompletedAppointments / todayTotalAppointments) * 100).toFixed(0) : 0}%</p><p className="text-gray-500 text-sm">Taxa Conclusão</p></div>
        </div>
      </div>

      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 print:shadow-none print:border-gray-300 print:rounded-md print:break-inside-avoid">
          ...
          {professionalReports.map((prof, index) => (
            <div key={index} className="border-b border-gray-100 last:border-b-0 pb-4 last:pb-0">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center"><p className="text-gray-500 text-sm mb-1">Pacientes</p><p className="text-blue-600 font-semibold text-lg">{prof.patients}</p></div>
                <div className="text-center"><p className="text-gray-500 text-sm mb-1">Atendimentos</p><p className="text-blue-600 font-semibold text-lg">R$ {prof.attendanceValue.toFixed(0)}</p></div>
                <div className="text-center"><p className="text-gray-500 text-sm mb-1">Comissão</p><p className="text-blue-600 font-semibold text-lg">R$ {prof.commission.toFixed(0)}</p></div>
              </div>
            </div>
          ))}
