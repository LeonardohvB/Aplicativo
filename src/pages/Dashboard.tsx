...
const Dashboard: React.FC = () => {
  ...
  const todayAppointments = slots.filter(slot => 
    slot.date === todayDate && 
    ...
    <div className="p-6 pb-24 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-1">
          Seja bem-vindo ao<br />seu consultório Ideiva
        </h1>
      </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        slot.status === 'agendado' ? 'bg-blue-100 text-blue-700' :
                        slot.status === 'em_andamento' ? 'bg-yellow-100 text-yellow-700' :
                        'bg-green-100 text-green-700'
                      }`}>
                        {slot.status === 'agendado' ? 'Agendado' :
                         slot.status === 'em_andamento' ? 'Em Andamento' :
                         'Concluído'}
                      </span>
                    </div>
                  </div>
